...
GeoLoc checkGPSRemote() { // remove 'Remote' if not working 
...
void driveTo(struct GeoLoc &loc, int timeout) {
  ...
  GeoLoc remoteLoc = checkGPSRemote(); //GeoLoc coolerLoc = checkGPS();
  char gpsData = nss.read();

  //bluetoothSerial.listen();

 if (remoteLoc.lat != 0 && remoteLoc.lon != 0 && enabled) { //if (coolerLoc.lat != 0 && coolerLoc.lon != 0 && enabled) {    float d = 0;
    //Start move loop here
    float d =0; 
    do {
      nss.read();
      remoteLoc = checkGPSRemote(); //coolerLoc = checkGPS();
...
void setup ()
{
  ...
  client.connect("192.168.4.1", 80); // IP address of the server
}
